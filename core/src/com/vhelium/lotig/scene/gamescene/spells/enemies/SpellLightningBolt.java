...
public class SpellLightningBolt extends SpellEnemy
{
	public static String name = "Lightning Bolt";
	private final String bulletAsset = "Lightning Bolt";
	...
	public SpellLightningBolt(int priority, HashMap<Integer, Float> conditions)
	{
	...
	public void activate(Realm realm, int shooterId, float x, float y, float rotation, float dirX, float dirY)
	{
		...
		realm.shoot(shooterId, true, DamageType.Lightning, getRange(level), x - GameHelper.getInstance().getBulletAsset(bulletAsset).getWidth() / 2 + dirX * 16, y - GameHelper.getInstance().getBulletAsset(bulletAsset).getHeight() / 2 + dirY * 16, rotation, dirX, dirY, bulletAsset, effect);
	}
	
	@Override
	public String getCastSound()
	{
		return SoundFile.electric;
	}
