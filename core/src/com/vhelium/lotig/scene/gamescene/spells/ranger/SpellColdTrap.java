...
public class SpellColdTrap extends Spell
{
	public static String name = "Cold Trap";
	public static String asset = "cold_trap";
	public static String effectAsset = "Cold Trap";
	public static String debuffAsset = "Frozen";
	public static String soundEffect = SoundFile.frost_blast;
	...
	public static int freezeDurationBase = 4000;
	public static int freezeDurationPerLevel = 250;
	
	public static final int width = 20;
	public static final int height = 20;
	
	public SpellColdTrap()
	{
		instantCast = true;
	}
	...
	{
		return "You place a cold trap on the ground. If an enemy steps on it it explodes damaging and freezing nearby enemies on the ground.\n\nDamage: " + getDamage(getLevel()) + " - " + (getDamage(getLevel()) + getBonusDamage(getLevel())) + "\nRange: " + getRange(getLevel()) + "\nFreeze duration: " + getFreezeDuration(getLevel()) + "\nTrap duration: " + getDuration(getLevel());
	}
	...
	public static void activate(int shooterId, int spellLevel, DataPacket dp, Realm realm)
	{
		...
		realm.addLevelObjectAtRuntime(new LevelObject_Trap(-1, realm, new Rectangle(x - width / 2, y - height / 2, width, height), asset, soundEffect, animated, animationTime, effectAsset, getDuration(level), realm.getEntity(shooterId).getTeamNr(), getDamage(level) + GameHelper.getInstance().getRandom().nextInt((int) getBonusDamage(level) + 1), getRange(level), DamageType.Cold, new Condition(name, "Root", 0, getFreezeDuration(level), false, debuffAsset, 0)));
	}
	...
	{
		return freezeDurationBase + freezeDurationPerLevel * (level - 1);
	}
